  ...
  const handleDisconnectHandle = async (handleName: string) => {
    ...
    try {
      const res = await fetch('/api/content/channel', {
        ...
        body: JSON.stringify({ handle_name: handleName, channel: 'newsletter' })
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <SidebarLayout>
      <div className="animate-fade-in" style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <section className="mobile-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '32px' }}>
